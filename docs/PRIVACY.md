...
The integration processes the Vikunja connection URL and API token, project and category metadata, task content and status, due dates, and task attachments. It also checks whether the current Home Assistant websocket user is an administrator for project/category management.

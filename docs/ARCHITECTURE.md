...
`dashboard.py` exposes three authenticated websocket commands:

- `vikunja/dashboard/get` returns the projects, categories, tasks, recurrence fields, and attachment metadata visible to the configured token.
...
- `vikunja/dashboard/action` performs task, project, category, bulk, recurrence, and attachment actions.

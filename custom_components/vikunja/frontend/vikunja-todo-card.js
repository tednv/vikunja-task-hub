      ...
      <ha-card class="${this._loading ? "busy" : ""}">
        ...
        ${
          !data
            ...
            : `
          <div class="list active-list">${active.map((task) => this._taskRow(task)).join("")}<div class="empty active-empty" ${active.length ? "hidden" : ""}>No active tasks.</div></div>
          <details class="completed"><summary>Completed (<span class="completed-count">${completed.length}</span>)</summary><div class="list">${completed.map((task) => this._taskRow(task)).join("")}<div class="empty completed-empty" ${completed.length ? "hidden" : ""}>No completed tasks.</div></div></details>
        `
        }
      </ha-card>
